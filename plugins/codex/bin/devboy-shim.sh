...
# have used:
#   1. ${CODEX_PLUGIN_DATA}/bin/devboy — GitHub Release fallback (offline / no npm)
...
if [ -n "${CODEX_PLUGIN_DATA:-}" ] && [ -x "$CODEX_PLUGIN_DATA/bin/devboy" ]; then
  exec "$CODEX_PLUGIN_DATA/bin/devboy" "$@"
fi
