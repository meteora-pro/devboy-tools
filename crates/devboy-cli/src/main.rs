...
"#;
    ...
    fn cli_markdown_renderer_emits_expected_landmarks() {
        ...
        // The `docs cli` subcommand itself must appear, otherwise the
        // refresh path advertised in the preamble is broken.
        assert!(
            body.contains("`devboy docs cli`"),
            "`devboy docs cli` heading missing from CLI reference"
        );
        ...
        for required in ["devboy init", "devboy tools docs", "devboy skills"] {
            assert!(
                ...
                "expected `{required}` in CLI reference, scan around `{}`",
