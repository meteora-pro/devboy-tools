...
fn render_manifest(proposals: &[ProposedPath]) -> String {
    ...
    // Deliberately no longer invites `approve_on_use`: the
    // per-use approval dialog is not wired in a shipped build,
    // so a path that takes the invitation becomes unresolvable.
    out.push_str("# Edit by hand to change paths or mark them optional.\n\n");
