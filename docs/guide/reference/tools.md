...
DevBoy Tools v0.35.0 ships 55 provider-backed tools across 7 categories, 14 always-on context tools, and 10 providers.
...
Run an existing manual GitLab CI job after verifying that it belongs to the supplied pipeline and is still manual.

| Parameter | Type | Required | Description |
|---|---|:---:|---|
| `jobId` | string | ✅ | Manual job ID returned by get_pipeline |
| `pipelineId` | string | ✅ | Pipeline ID returned by get_pipeline |
| `jobInputs` | object | — | Optional typed GitLab job inputs as an object of JSON values |
| `variables` | object | — | Optional CI variables as an object whose values are strings |
...
Re-open the secret vault with a six-digit code from the user's authenticator (ADR-024 §1). Use this when a secret resolve failed because the vault is locked: ask the user for the current code, relay it here, then retry the original call. The code proves a human approved the re-unlock — you cannot derive one yourself.

| Parameter | Type | Required | Description |
|---|---|:---:|---|
| `totp` | string | ✅ | The six-digit code the user reads from their authenticator. |
| `duration_seconds` | number | — | How long the unlock should last. The daemon clamps this to the user's configured ceiling, so asking for more than they allowed does not widen it. |

### `secrets_validate`

Check whether the secret stored at a path is the shape it is supposed to be (ADR-024 §3). Use it right after a value is provisioned or rotated, to catch the pasted-the-wrong-thing case before it fails somewhere confusing. Everything happens in the secret daemon and only a verdict comes back — you name the path, you never see the value. There is deliberately no way to supply your own pattern: the check runs against the rule recorded with the entry. A `format` of `unknown` means nobody declared a shape for that path, which is not the same as passing.
...
What the vault will accept right now: whether it is unlocked, how long the current window has left, and which unlock methods are available. Check this before asking the user for anything — it is the difference between "the vault is locked, give me a code" and asking for a code nobody needs to type.
