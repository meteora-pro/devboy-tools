...
Skill names match the source filenames one-to-one — `plugins/claude/skills/<name>` is a symlink to `skills/<category>/<name>/`, so editing `/skills/00-self-bootstrap/setup/SKILL.md` updates the plugin in the same edit. Plugin namespacing prepends `devboy:`, so:
