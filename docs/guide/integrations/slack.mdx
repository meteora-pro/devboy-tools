...
1. **Daily summary** — `devboy-chat-summary` skill summarises the last 24 h of a channel for a stand-up note.
2. **Notification** — `devboy-notify` posts a structured message after a long-running CI / deploy.
3. **Decision archaeology** — `devboy-chat-search` lets the agent grep team channels for "why did we choose Postgres over MySQL".
