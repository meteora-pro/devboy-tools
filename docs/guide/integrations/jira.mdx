...
| Tool | Description |
|------|-------------|
| `list_project_versions` | List versions for a project. Returns `id`, `name`, `description`, `releaseDate`, `startDate`, `released`, `archived`, `overdue`, and (Cloud only, when `includeIssueCount=true`) `issue_count`. Default filter hides archived versions and caps at 20 most recent — unreleased first, then released by `releaseDate desc`, semver-aware tiebreak on name. |
| `upsert_project_version` | Create-or-update a version keyed by `(project, name)`. Partial update — fields you omit are preserved server-side. Useful for writing release notes (`description`) or closing a release (`released: true`, `releaseDate`). |
