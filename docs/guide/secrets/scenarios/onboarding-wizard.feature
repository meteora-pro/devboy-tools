...
Feature: setup-secrets wizard onboards a fresh project
  ...
  Scenario: Re-running with --resume picks up where the wizard left off
    ...
    Then the wizard resumes at the first phase that is not done
    And the Scan and Propose phases are skipped with the reason "already done"
  ...
  Scenario: Catalog-driven proposer accuracy on a real project
    ...
    Then the proposed-path count is materially lower than the pre-skip-list
    baseline (no fixture pins the exact number — see the @not-covered
    scenario in proposer-noise-reduction.feature)
