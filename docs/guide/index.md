...
- **Research-driven.** Every optimisation in the pipeline traces back to a paper grounded in a real corpus — 523 Claude Code sessions, 10,644 MCP tool responses. We don't ship a heuristic without measuring it. See [research index](https://github.com/meteora-pro/devboy-tools/blob/main/docs/research/INDEX.md).
...
|-|------------------------|--------|
| **Output efficiency** | Default API JSON | Knapsack-based pagination + format-adaptive encoding ([papers 1, 2](https://github.com/meteora-pro/devboy-tools/blob/main/docs/research/INDEX.md)) — measured per-call savings on the 144k-event corpus: 69% avg on `get_issues`, 92% top per call, 26% avg on `*_pipeline`. KV-cache pass on Sonnet 4.5 lifts ~40% of tokens off the input side. |
...
**OpenCode** and **Kimi CLI** users get the same skills for free — both auto-read `~/.claude/skills/`, so installing the Claude Code plugin or running `devboy onboard` covers them too. See the per-agent guides ([Claude Code](./integrations/claude-code-plugin.mdx), [Codex](./integrations/codex-plugin.mdx)) and [ADR-018](https://github.com/meteora-pro/devboy-tools/blob/main/docs/architecture/adr/ADR-018-plugin-distribution.md) for the architecture.
...
Adding a provider is a Rust crate implementing `Provider` + a `ToolEnricher` ([ADR-007](https://github.com/meteora-pro/devboy-tools/blob/main/docs/architecture/adr/ADR-007-plugin-architecture.md)).
...
- **Manifest-driven** — projects declare required and optional secrets in `.devboy/secrets.toml` ([ADR-020](https://github.com/meteora-pro/devboy-tools/blob/main/docs/architecture/adr/ADR-020-secret-manifest-and-alias-resolution.md)). The merged inventory is the source of truth for `secrets list`, `doctor`, and the inventory view.
- **Pluggable sources** — keychain, local-vault, 1Password, Vault (HTTP KV v2), env-store ship in-tree; community plugins extend the set via a stdio JSON-RPC subprocess protocol ([ADR-021](https://github.com/meteora-pro/devboy-tools/blob/main/docs/architecture/adr/ADR-021-secret-source-router.md)).
- **Native UI** — TUI on ratatui + GUI on egui sharing one view-model layer; backend autodetected from `$DISPLAY`/`$WAYLAND_DISPLAY` ([ADR-023](https://github.com/meteora-pro/devboy-tools/blob/main/docs/architecture/adr/ADR-023-secret-store-ux-layer.md) §3.4).
...
For AI-driven setup, point the agent at the [`setup-secrets` skill](https://github.com/meteora-pro/devboy-tools/blob/main/crates/devboy-skills/skills/00-self-bootstrap/setup-secrets/SKILL.md) — it walks the eight-step wizard with a state file at `~/.devboy/secrets/setup-state.toml`.

---

## Research

Every non-trivial optimisation in the pipeline is backed by a paper grounded in a real corpus — 523 Claude Code sessions, 10,644 MCP responses from production traffic. The full [`docs/research/INDEX.md`](https://github.com/meteora-pro/devboy-tools/blob/main/docs/research/INDEX.md) tracks methods, datasets, and reproducibility scripts.
...
| [`devboy-skills`](https://github.com/meteora-pro/devboy-tools/blob/main/crates/devboy-skills) | Skills subsystem (SKILL.md parser, install lifecycle) | [![Crates.io](https://img.shields.io/crates/v/devboy-skills.svg)](https://crates.io/crates/devboy-skills) | [![Docs](https://docs.rs/devboy-skills/badge.svg)](https://docs.rs/devboy-skills) |
| [`devboy-cli`](https://github.com/meteora-pro/devboy-tools/blob/main/crates/devboy-cli) | The `devboy` CLI binary (npm is the primary channel) | [![Crates.io](https://img.shields.io/crates/v/devboy-cli.svg)](https://crates.io/crates/devboy-cli) | [![Docs](https://docs.rs/devboy-cli/badge.svg)](https://docs.rs/devboy-cli) |
...
The release procedure is documented in [`docs/guide/contributing/release.md`](./contributing/release). See [ADR-022](https://github.com/meteora-pro/devboy-tools/blob/main/docs/architecture/adr/ADR-022-crates-io-publishing.md) for the architectural decision behind the dual npm + crates.io distribution.
...
See [CONTRIBUTING.md](https://github.com/meteora-pro/devboy-tools/blob/main/CONTRIBUTING.md) for the full guide (commit conventions, branch naming, ADR workflow, release process).
...
[Apache License 2.0](https://github.com/meteora-pro/devboy-tools/blob/main/LICENSE) — use it, modify it, ship it; if you build something interesting on top, we'd love a heads-up via Discussions.
