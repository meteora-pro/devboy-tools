...
```bash
# Set the ClickUp team (workspace) and list IDs (found in the list URL)
...
:::tip
The `team_id` is required for **Custom Task IDs** (e.g., `DEV-42`). Without it, only `CU-{id}` keys are supported.
:::
